...
const Events = () => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarDays.push(<div key={`day-${day}`} className={`h-10 w-10 flex items-center justify-center rounded-full cursor-pointer transition-colors ${isToday ? "bg-primary text-white" : hasEvent ? "hover:bg-primary/20 relative" : "hover:bg-gray-100"}`}>
        {day}
        {hasEvent && <span className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-primary"></span>}
      </div>);
  }
  return <section id="events" className="section-padding bg-white">
      <div className="container-center">
        <div className="text-center mb-12">
          ...
          <p className="max-w-3xl mx-auto text-lg">Get the latest updates on news, events, and achievements.</p>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="bg-accent rounded-lg p-6 shadow-md max-w-md mx-auto px-[24px]">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {daysOfWeek.map(day => <div key={day} className="h-10 flex items-center justify-center font-medium text-sm text-gray-500">
                  {day}
                </div>)}
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {newsItems.map((item, index) => <motion.div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200" initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            opacity: 1,
            ...
            once: true
                ...
                <div className="p-6">
                  ...
                  <a href="#" className="text-primary font-medium inline-flex items-center hover:text-accent-red transition-colors">
                    Read More
