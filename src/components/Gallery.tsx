...
const Gallery = () => {
  ...
  return <section id="gallery" className="section-padding bg-accent">
      <div className="container-center">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Gallery</h2>
          <div className="mx-auto w-20 h-1 bg-primary mb-6"></div>
          <p className="text-lg max-w-3xl mx-auto">Explore our school through images of campus life and events.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredImages.map((image, index) => <motion.div key={image.id} className="overflow-hidden rounded-lg shadow-md bg-white cursor-pointer h-[250px]" whileHover={{
          y: -5
        }} onClick={() => setSelectedImage(image.id)} layoutId={`gallery-image-${image.id}`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
        ...
        {/* Lightbox */}
        <AnimatePresence>
          {selectedImage !== null && <motion.div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          opacity: 0
              ...
              <motion.div className="relative max-w-4xl w-full bg-white rounded-lg overflow-hidden" layoutId={`gallery-image-${selectedImage}`}>
                <button className="absolute top-4 right-4 bg-white rounded-full p-1 shadow-md z-10" onClick={() => setSelectedImage(null)}>
                  <X size={20} />
                </button>
                
                <div className="w-full aspect-video">
                  {galleryImages.find(img => img.id === selectedImage)?.src && <img src={galleryImages.find(img => img.id === selectedImage)?.src} alt={galleryImages.find(img => img.id === selectedImage)?.alt || "Gallery image"} className="w-full h-full object-cover" />}
                </div>
